...
const animationEngine = ( () => {
  ...
  /* The above code is implementing an AnimationEngine class in JavaScript. This class manages animations
  by adding and removing them, and updating them using requestAnimationFrame. It keeps track of
  animation IDs and their corresponding animation objects. The AnimationEngine class also calculates
  the time delta between animation updates to ensure smooth animation playback. */
    let uniqueID = 0;
  ...
  class Animation {
    ...
    /* The Animation class is a JavaScript class that can be started and stopped, and has an update method
  for handling animation updates. */
  ...
  /* The World class extends Animation and sets up a 3D scene with lights, camera, and rendering
  capabilities for a game. */
    constructor( game ) {
  ...
  /**
   * The `RoundedBoxGeometry` function creates a custom geometry for a rounded box shape in Three.js.
   * @param size - The `size` parameter in the `RoundedBoxGeometry` function represents the overall size
   * of the rounded box in three dimensions (width, height, depth). It is used to determine the
   * dimensions of the box and calculate the radius based on the size provided.
   * @param radius - The `radius` parameter in the `RoundedBoxGeometry` function represents the radius of
   * the rounded corners of the box geometry. It is used to determine the curvature of the corners.
   * @param radiusSegments - The `radiusSegments` parameter in the `RoundedBoxGeometry` function
   * determines the number of segments used to create the rounded corners of the box geometry. Increasing
   * the `radiusSegments` value will result in smoother and more detailed rounded corners on the box.
   */
  ...
  /**
   * The function `RoundedPlaneGeometry` creates a rounded plane geometry with specified size, radius,
   * and depth using Three.js.
   * @param size - The `size` parameter represents the overall size of the rounded plane geometry. It is
   * used to calculate the dimensions and radius of the rounded corners.
   * @param radius - The `radius` parameter in the `RoundedPlaneGeometry` function represents the radius
   * of the rounded corners of the plane geometry. It is used to determine how rounded the corners of the
   * plane will be.
   * @param depth - The `depth` parameter in the `RoundedPlaneGeometry` function represents the depth or
   * thickness of the extruded geometry. It determines how far the 2D shape is extruded in the third
   * dimension to create a 3D object. This parameter controls the distance from the front face to the
   * @returns The function `RoundedPlaneGeometry` returns a `THREE.ExtrudeBufferGeometry` object that
   * represents a rounded plane geometry based on the input parameters `size`, `radius`, and `depth`. The
   * geometry is created using a `THREE.Shape` with rounded corners defined by the specified radius.
   */
  ...
  /* The Cube class in JavaScript defines a 3D cube object with customizable size, geometry, and colors
  ...
  const Easing = {
    /* The above code defines an object named `Easing` which contains different easing functions for
  animations. Each easing function is categorized under different types such as Power, Sine, Back, and
  Elastic. */
